...
class Car {
    ...
    bool out(Vector2i ws) {
        return (m_px < camtile.y - 7) || (m_py > camtile.y + ws.y / wts + 8) || (m_px < camtile.x - 7) || (m_px > camtile.x + (ws.x) / wts + 8);
    };
    ...
    void trip(World& world, vector<Vector2i> parks, vector<Vector2i> shops, vector<Vector2i> fuels) {
        if (queue.empty()) {
            ...
            // add pause
            //printf("%d %d\n", pos.x, pos.y);
            queue.push_back(Action(Action::Idle));
            switch (type) {
                case 1:
                queue.push_back(Action(Action::Wait, rint(4 * 60, 10 * 60)));
                break;
                case 2:
                queue.push_back(Action(Action::Wait, rint(6 * 60, 20 * 60)));
                break;
                case 3:
                queue.push_back(Action(Action::Wait, 5 * 60));
                break;
    ...
    void move(World& world) {
        ...
        switch (act.id) {
            ...
            break;
            case Action::Idle: {
                Tile* tile = world.ftile(Vector2i(m_ox, m_oy));
                if (tile)
                    tile->buf.occ = false;
    ...
    // render car
    bool render(VertexArray& arr, Vector2i mv) {
        renderSquare(arr, IntRect(m_x * wts / 64, m_y * wts / 64, wts, wts), texCar[m_look], m_d);
        return IntRect(m_x * wts / 64, m_y * wts / 64, wts, wts).contains(mv);
    };

    // follow car
    void follow() {
        camera.x = (m_px * 64 + m_dx) * wts / 64 + 32;
        camera.y = (m_py * 64 + m_dy) * wts / 64 + 32;
    };
    ...
    int speed() {
        ...
        // get maximum speed
        int s = m_speed * m_road / 255;
        int r = s * m_accel / 12.0;
    ...
    // get data
    Vector2i getServices() {
        return Vector2i(m_rest, m_fuel);
    };
    // get speed
    Vector3i getSpeed() {
        int ac = m_speed * m_road * m_accel / 255.0 / 12;
        return Vector3i(m_speed, m_road, m_a ? (ac == 0 ? 1 : ac) : 0);
    };
    // get coords
    Vector2i getCoords() {
        return Vector2i(m_px * 64 + m_dx, m_py * 64 + m_dy);
    };
    // get waiting time
    int getWait() {
        if (queue.empty())
        ...
        Action& act = queue.back();
        if (act.id == Action::Wait)
            return act.param;
